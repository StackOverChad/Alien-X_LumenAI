...
export default function UserDashboard() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Add user message to chat history with stable id
    setChatHistory(prev => [...prev, { id: `m-${Date.now()}-${Math.random().toString(36).slice(2,8)}`, role: 'user', content: userMessage }]);
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {availableFiles.map((file, idx) => {
                    const key = file.id ?? file.name ?? `file-${idx}`;
                    const value = file.id ?? file.name ?? `file-${idx}`;
                    return (
                      <option key={key} value={value}>{file.name}</option>
                    );
                  })}
                </select>
              </div>
